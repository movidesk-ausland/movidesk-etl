        ...
        while tentativas < 3:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Erro {response.status_code} na pagina {page + 1}, tentativa {tentativas + 1}/3")
                    tentativas += 1
                    import time; time.sleep(5)
            except Exception as e:
                print(f"Excecao na pagina {page + 1}, tentativa {tentativas + 1}/3: {e}")
                tentativas += 1
                import time; time.sleep(5)

        if not sucesso:
            print(f"Pagina {page + 1} falhou apos 3 tentativas, pulando...")
            page += 1
            continue
...
params_1 = {
    ...
    "$select": "id,subject,tags,serviceFull,createdDate,lastActionDate,owner,ownerTeam,baseStatus,status,category,clients,origin,actions",
    "$expand": "actions($select=origin,createdDate,description),clients($select=businessName;$expand=organization($select=businessName)),owner($select=businessName)",
...
for ticket in all_data[:3]:
    actions = ticket.get('actions', [])
    if actions:
        print("Campos disponíveis na action:", actions[0].keys())
        print("Conteúdo da primeira action:", actions[0])
        break
# ============================================================
...
def flatten_actions(ticket):
    ...
    flat_ticket['first_action_description'] = None

    if actions:
        actions_sorted = sorted(actions, key=lambda x: x.get("createdDate"))
        # Primeira acao com id == 1 ou simplesmente a primeira da lista
        first = actions_sorted[0] if actions_sorted else None
        if first:
            flat_ticket['first_action_description'] = first.get("description")
...
colunas = [
    "id", "subject", "tags", "service_full", "created_date", "last_action",
